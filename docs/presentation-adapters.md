...
2. Refatorar `createFieldTemplate` e `createObjectFieldTemplate` em
   ...
   duplicar labels.
3. Adicionar a prop `presentation` a `AFormProps`, contendo registry e adapter
...
4. Exportar os contratos em `packages/react/src/index.ts` e documentar que os
...
1. Criar `packages/presentation-material/` com versoes compativeis de
...
2. Criar `packages/presentation-bootstrap/` usando `@rjsf/react-bootstrap` e
   ...
   `<link>` em runtime.
3. Criar `packages/presentation-tailwind/` sem introduzir dependencia de
   ...
   e foco.
4. Criar `packages/presentation-neobrutalism/` com templates, widgets e CSS
...
1. Em `apps/playground-v2/src/builder/BuilderApp.tsx`, criar um registry estavel
...
3. Atualizar `apps/playground-v2/package.json`, os quatro novos `package.json` e
...
4. Atualizar `packages/react/README.md`, os READMEs dos adapters, o README raiz e
   os changelogs. Explicar a separacao entre spec e runtime, a criacao de um
   adapter custom, os imports de CSS e providers e a matriz de suporte. Registrar
   que `ui:widget` escolhe a intencao do controle, enquanto o adapter escolhe
   DOM, animacao e aparencia.
...
1. Executar primeiro os testes focados de `a-form-react`, depois os testes dos
   quatro adapters e do model e playground.
2. Executar `npm run typecheck` para validar os genericos RJSF e as referencias
   entre workspaces.
3. Executar `npm test` e `npm run build` no monorepo.
